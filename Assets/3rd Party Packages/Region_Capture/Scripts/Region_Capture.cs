...
public class Region_Capture : MonoBehaviour
{
    ...
    private void Initialize()
    {
        ...
        if (VuforiaRenderer.Instance.IsVideoBackgroundInfoAvailable())
        {
			...
			// Original Code gives an Texture to Texture2D cast error, replaced it with following code.
            //VideoBackgroundTexure = VuforiaRenderer.Instance.VideoBackgroundTexture;
			Texture2D PreVideoBackgroundTexure = (VuforiaRenderer.Instance.VideoBackgroundTexture as Texture2D);
			VideoBackgroundTexure = PreVideoBackgroundTexure;
